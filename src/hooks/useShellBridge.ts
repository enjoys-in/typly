...
export function useShellBridge(): void {
  ...
  useEffect(() => {
    const open = (file: OpenedFile) => {
      // A challenge carries its own rules and a score to beat, so it skips
      // import entirely. An unreadable one falls through to the ordinary path,
      // so a mis-named text file still opens as a passage rather than vanishing.
      if (acceptChallenge(file)) return;
      setFile(file);
      navigate('/app/new');
    };
    ...
    return off;
  }, [platform, navigate, setFile, acceptChallenge]);
